...
test('four-route index, navigation, focus, motion and accessibility gates', {timeout:180000},async(t)=>{
  ...
  try {
    for(const [width,height] of sizes) await t.test(`home ${width}x${height}`,async()=>{
      ...
      report.viewports.push({width,height,...computed,sheetWidth,headings:{h1:1,h2:1,h3:4},indexEntries:4,axeViolations:0,consoleErrors:0,externalRequests:1,reducedMotionSeconds:duration});
      await context.close();
    });
